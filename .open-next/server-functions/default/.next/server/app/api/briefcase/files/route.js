(()=>{var a={};a.id=6066,a.ids=[4291,6066],a.modules={261:a=>{"use strict";a.exports=require("next/dist/shared/lib/router/utils/app-paths")},3295:a=>{"use strict";a.exports=require("next/dist/server/app-render/after-task-async-storage.external.js")},10846:a=>{"use strict";a.exports=require("next/dist/compiled/next-server/app-page.runtime.prod.js")},19121:a=>{"use strict";a.exports=require("next/dist/server/app-render/action-async-storage.external.js")},29294:a=>{"use strict";a.exports=require("next/dist/server/app-render/work-async-storage.external.js")},30477:(a,b,c)=>{"use strict";c.d(b,{HG:()=>i,JE:()=>g,fH:()=>h,vF:()=>e,vV:()=>f});class d{constructor(){this.isDevelopment=!1,this.logLevel=this.isDevelopment?3:2}shouldLog(a){return a<=this.logLevel}formatLogEntry(a){let b=["ERROR","WARN","INFO","DEBUG"][a.level],c=`[${a.timestamp}] ${b}: ${a.message}`;return a.context&&Object.keys(a.context).length>0&&(c+=` | Context: ${JSON.stringify(a.context)}`),a.error&&(c+=` | Error: ${a.error.message}`,this.isDevelopment&&a.error.stack&&(c+=` | Stack: ${a.error.stack}`)),c}log(a,b,c,d){if(!this.shouldLog(a))return;let e={level:a,message:b,timestamp:new Date().toISOString(),context:c,error:d},f=this.formatLogEntry(e);0===a&&console.error(f),!this.isDevelopment&&a<=1&&this.sendToExternalService(e)}async sendToExternalService(a){}error(a,b,c){this.log(0,a,b,c)}warn(a,b){this.log(1,a,b)}info(a,b){this.log(2,a,b)}debug(a,b){this.log(3,a,b)}apiLog(a,b,c,d,e){let f=`${a} ${b} - ${c}${d?` (${d}ms)`:""}`,g=c>=500?0:c>=400?1:2;this.log(g,f,{method:a,path:b,statusCode:c,duration:d,...e})}dbLog(a,b,c,d){let e=`DB ${a} on ${b}${c?` (${c}ms)`:""}`;this.log(3,e,{operation:a,table:b,duration:c,...d})}authLog(a,b,c=!0,d){let e=`Auth ${a}${b?` for user ${b}`:""} - ${c?"SUCCESS":"FAILED"}`;this.log(c?2:1,e,{action:a,userId:b,success:c,...d})}}let e=new d,f=(a,b,c)=>{b instanceof Error?e.error(a,void 0,b):"object"==typeof b&&null!==b?e.error(a,b,c):e.error(a,void 0,c)},g=(a,b)=>e.warn(a,b),h=(a,b)=>e.info(a,b),i=(a,b,c,d,f)=>e.apiLog(a,b,c,d,f)},44870:a=>{"use strict";a.exports=require("next/dist/compiled/next-server/app-route.runtime.prod.js")},63033:a=>{"use strict";a.exports=require("next/dist/server/app-render/work-unit-async-storage.external.js")},67944:(a,b,c)=>{"use strict";c.r(b),c.d(b,{handler:()=>I,patchFetch:()=>H,routeModule:()=>D,serverHooks:()=>G,workAsyncStorage:()=>E,workUnitAsyncStorage:()=>F});var d={};c.r(d),c.d(d,{GET:()=>B,POST:()=>C,dynamic:()=>z});var e=c(95736),f=c(9117),g=c(4044),h=c(39326),i=c(32324),j=c(261),k=c(54290),l=c(85328),m=c(38928),n=c(46595),o=c(3421),p=c(17679),q=c(41681),r=c(63446),s=c(86439),t=c(51356),u=c(30477),v=c(10641),w=c(74291),x=c(9608),y=c(73927);let z="force-dynamic";function A(){let a="postgresql://neondb_owner:npg_MRLUf85DBNPv@ep-curly-meadow-aefoagku-pooler.c-2.us-east-2.aws.neon.tech/neondb?sslmode=require&channel_binding=require";if(!a)throw Error("DATABASE_URL is not set");return(0,x.lw)(a)}async function B(a){try{let{user:b,error:c}=await (0,w.authenticateRequest)();if(c||!b)return v.NextResponse.json({error:"Unauthorized"},{status:401});let d=new URL(a.url),e=parseInt(d.searchParams.get("page")||"1"),f=parseInt(d.searchParams.get("limit")||"20"),g=d.searchParams.get("category"),h=d.searchParams.get("search"),i=d.searchParams.get("folderId"),j=A(),k=(e-1)*f,l=await j`
      ...
      FROM documents 
      WHERE user_id = ${b.id}
        ${g&&"all"!==g?j`AND category = ${g}`:j``}
        ${h?j`AND (name ILIKE ${`%${h}%`} OR description ILIKE ${`%${h}%`})`:j``}
        ${i?j`AND folder_id = ${i}`:j``}
    ...
    `,m=await j`
      ...
      WHERE user_id = ${b.id}
        ...
        ${h?j`AND (name ILIKE ${`%${h}%`} OR description ILIKE ${`%${h}%`})`:j``}
        ${i?j`AND folder_id = ${i}`:j``}
    `,n=parseInt(m[0]?.total||"0");return v.NextResponse.json({documents:l,pagination:{page:e,limit:f,total:n,totalPages:Math.ceil(n/f)}})}catch(a){return(0,u.vV)("Error fetching documents:",a),v.NextResponse.json({error:"Failed to fetch documents"},{status:500})}}async function C(a){try{let{user:b,error:c}=await (0,w.authenticateRequest)();if(c||!b)return v.NextResponse.json({error:"Unauthorized"},{status:401});let d=await a.formData(),e=d.getAll("files"),f=d.get("folderId");if(!e||0===e.length)return v.NextResponse.json({error:"No files provided"},{status:400});let g=A(),h=[];for(let a of e){let c=crypto.randomUUID(),d=a.name.split(".").pop()?.toLowerCase()||"unknown",e="uncategorized";["jpg","jpeg","png","gif","webp","svg"].includes(d)?e="images":["pdf","doc","docx","txt","md"].includes(d)?e="documents":["mp4","avi","mov","wmv"].includes(d)?e="videos":["mp3","wav","ogg","m4a"].includes(d)&&(e="audio");try{let i=await (0,y.QM)(a,`${c}-${a.name}`,b.id),j=await g`
