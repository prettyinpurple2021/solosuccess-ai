...
export const AgentChat: React.FC<AgentChatProps> = ({ agentId, initialMessage, onMessageConsumed }) => {
    ...
    return (
        ...
        <div className="flex flex-col h-[calc(100vh-8rem)] md:h-[calc(100vh-4rem)] max-h-[800px] glass-panel rounded-xl overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-300 relative">

            {/* Agent Header & Tabs */}
            <div className="bg-white/5 border-b border-white/5 backdrop-blur-md relative z-20">
                <div className="p-4 md:p-6 flex items-center justify-between gap-4">
                    <div className="flex items-center gap-4">
                        <div className="relative">
                            <img src={agent.avatar} alt={agent.name} className="w-10 h-10 md:w-12 md:h-12 rounded-full grayscale hover:grayscale-0 transition-all ring-2 ring-white/10" />
                            <div className={`absolute bottom-0 right-0 w-3.5 h-3.5 rounded-full border-2 border-zinc-950 ${agent.color.replace('text-', 'bg-')} shadow-[0_0_10px_currentColor]`} />
                        </div>
                        <div>
                            <h3 className={`font-black text-xl md:text-2xl tracking-tight ${agent.color} drop-shadow-md`}>{agent.name.toUpperCase()}</h3>
                            <p className="text-[10px] md:text-xs text-zinc-400 font-mono uppercase tracking-widest">{agent.title}</p>
                        </div>
                    </div>

                    <div className="flex items-center gap-3">
                        {/* Deep Mind Indicator */}
                        <div className="hidden md:flex items-center gap-2 px-3 py-1.5 bg-black/40 rounded-full border border-white/10 group relative cursor-help hover:border-emerald-500/50 transition-colors">
                        ...
                        <button
                            ...
                            className="p-2 rounded hover:bg-red-500/10 text-zinc-500 hover:text-red-500 transition-colors border border-transparent hover:border-red-500/20"
                ...
                <div className="flex px-4 md:px-6 gap-1">
                    <button
                        ...
                        className={`flex items-center gap-2 px-4 py-3 text-xs font-bold uppercase tracking-widest border-b-2 transition-colors relative
                    ${activeTab === 'chat' ? `border-${agent.color.split('-')[1]}-500 text-white` : 'border-transparent text-zinc-500 hover:text-zinc-300'}
                `}
                    >
                        <MessageSquare size={14} /> Uplink
            ...
            {activeTab === 'chat' && (
                <>
                    <div className="flex-1 overflow-y-auto p-4 space-y-6 relative custom-scrollbar z-10" ref={scrollRef}>
                        ...
                        {messages.map((msg, idx) => {
                            ...
                            return (
                                <div key={idx} className={`flex ${isUser ? 'justify-end' : 'justify-start'} relative z-10 animate-in slide-in-from-bottom-2 duration-300`}>
                                    <div className={`max-w-[90%] md:max-w-[80%] p-5 rounded-2xl shadow-lg backdrop-blur-sm ${isUser
                                        ...
                                        <div className="flex items-center gap-2 mb-2 text-[10px] font-mono uppercase opacity-50">
                                            {isUser ? <User size={10} /> : <Bot size={10} />}
                                            <span className="font-bold tracking-wider">{isUser ? 'YOU' : agent.name}</span>
                                            <span className="ml-auto">{new Date(msg.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                                        </div>
                                        <p className="text-sm whitespace-pre-wrap leading-relaxed font-light">{msg.text}</p>
                                    </div>
                                </div>
                            );
                        })}
                        {loading && (
                            <div className="flex justify-start relative z-10">
                                <div className="bg-black/40 border border-white/10 p-4 rounded-2xl rounded-tl-none flex items-center gap-3 backdrop-blur-sm">
                    ...
                    {/* Input */}
                    <div className="p-4 bg-black/20 border-t border-white/5 backdrop-blur-md relative z-20">
                        <div className="flex gap-3">
                            <input
                                type="text"
                                className="flex-1 bg-black/40 border border-white/10 text-white rounded-xl px-4 py-3 focus:outline-none focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/50 font-medium placeholder-zinc-600 transition-all"
                            ...
                            <button
                                ...
                                className="bg-white hover:bg-zinc-200 text-black p-3 rounded-xl disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-[0_0_15px_rgba(255,255,255,0.1)] hover:shadow-[0_0_20px_rgba(255,255,255,0.3)]"
                            >
                                <Send size={20} />
                            </button>
                        </div>
                ...
                <div className="flex-1 overflow-y-auto p-4 md:p-8 bg-black/20 custom-scrollbar">
                    {tasks.length === 0 ? (
                        <div className="h-full flex flex-col items-center justify-center text-zinc-600 opacity-50">
                            ...
                            <p className="mt-4 font-mono text-sm uppercase tracking-widest">No Active Directives for {agent.name}</p>
                            <p className="text-xs mt-2">Assign tasks in the Tactical Roadmap.</p>
                        </div>
                    ) : (
                        <div className="space-y-4 max-w-2xl mx-auto">
                            <div className="flex items-center justify-between mb-6">
                                <h3 className="text-sm font-bold text-white uppercase tracking-widest flex items-center gap-2">
                                    <AlertCircle size={16} className={agent.color} /> Pending Directives
                                </h3>
